%% Initialise workspace
clc;
clf;
clear all;

% Start timer
timer_start = tic;

%% Set up constants

% Number of fibres desired
fibre_count = 20;

% Number of nodes along each fibre (for the first pass)
% Use at least the number of key points
nodes_per_fibre_1 = 5;

% Accuracy of trim - Aim for 200 in final?
accuracy = 5;

% Spacing between nodes of Ranvier (in millimetres)
internode_ranvier_spacing = 0.050;

%% Input coordinates

% Rosenthal's canal
RC_points = [1.800 4.225 2.150;
1.750 4.000 2.150;
1.675 3.775 2.200;
1.600 3.575 2.350;
1.600 3.450 2.550;
1.700 3.350 2.775;
1.875 3.275 2.950;
2.075 3.250 3.075;
2.300 3.200 3.150;
2.550 3.150 3.150;
2.775 3.100 3.025;
2.900 3.050 2.800;
2.950 3.000 2.550;
2.800 2.925 2.350;
2.550 2.850 2.250;
2.325 2.700 2.350;
2.250 2.550 2.550;
2.325 2.400 2.775;
2.550 2.250 2.850;
2.775 2.125 2.775;
2.850 2.000 2.550];

% Peripheral processes
PP_points = [1.549 4.371 1.555; % 1.536 4.301 1.505; %1.517 4.227 1.495;
1.323 3.802 1.505;
1.073 3.446 1.825;
0.990 3.293 2.360;
1.115 3.237 2.910;
1.545 3.214 3.410;
2.215 3.131 3.600;
2.789 3.076 3.570;
3.173 3.011 3.165;
3.247 2.992 2.740;
3.154 2.951 2.350;
2.891 2.872 2.060;
2.544 2.775 1.975;
2.197 2.650 2.065;
1.980 2.516 2.345;
1.980 2.382 2.675;
2.146 2.271 2.945;
2.419 2.160 3.120;
2.706 2.086 3.115;
3.025 1.979 2.860;
3.066 1.919 2.580;
3.062 1.910 2.535];

% Axon (start, near RC)
A1_points = [1.924 4.223 2.340;
1.753 3.760 2.355;
1.855 3.441 2.755;
2.178 3.316 3.015;
2.590 3.242 3.060;
2.835 3.187 2.815;
2.849 3.085 2.495;
2.683 2.914 2.340;
2.405 2.761 2.350;
2.290 2.678 2.560;
2.390 2.502 2.790;
2.613 2.300 2.795;
2.790 2.210 2.670;
2.812 2.140 2.565];

% Axon (middle, near bend)
A2_points = [2.419 4.264 2.460;
2.345 4.280 2.545;
2.295 4.269 2.650;
2.322 4.223 2.750;
2.428 4.116 2.850;
2.539 4.056 2.900;
2.669 3.977 2.910;
2.835 3.908 2.860;
2.923 3.871 2.730;
2.951 3.857 2.605;
2.915 3.850 2.495;
2.854 3.840 2.420;
2.710 3.830 2.420;
2.595 3.810 2.445;
2.479 3.780 2.525;
2.460 3.750 2.675;
2.507 3.730 2.785;
2.627 3.750 2.825;
2.761 3.775 2.795;
2.784 3.800 2.695];
% 2.780 3.600 2.565];

% Axon (end)
A4_points = [3.617 4.967 2.365;
3.580 5.032 2.535;
3.547 5.097 2.710;
3.538 5.120 2.930;
3.598 5.004 3.080;
3.691 4.824 3.165;
3.788 4.639 3.200;
3.867 4.482 3.120;
3.945 4.329 2.940;
3.996 4.236 2.700;
3.964 4.301 2.505;
3.885 4.445 2.430;
3.797 4.625 2.335;
3.709 4.791 2.340;
3.631 4.944 2.530;
3.598 5.004 2.790;
3.635 4.926 2.975;
3.728 4.754 3.075;
3.811 4.593 3.055;
3.885 4.445 2.925;
3.941 4.343 2.755];
% 3.917 4.389 2.585];

% Axon (near end, offset)
shift_vector = [3.492 4.912 2.370] - [3.617 4.967 2.365];
A3_points = zeros(size(A4_points));

for i=1:size(A4_points,1)
    A3_points(i,:) = A4_points(i,:) + shift_vector;
end

%% Format plot

% Axes, title, etc.
axis equal;
grid on;
xlabel X;
ylabel Y;
zlabel Z;
title 'Interpolated nerve fibres';
hold on;

% Viewpoint
view([0 -90]);

%% MAKE SPIRAL SPLINES

%% Rosenthal's canal

% Calculate interpolated points
px = RC_points(:,1); % These are temporary variables for plotting points
py = RC_points(:,2);
pz = RC_points(:,3);
interp_points_RC = interparc(fibre_count, px, py, pz);

% Plot RC
plot3(px, py, pz);      % Original points (from manual selection)
plot3(interp_points_RC(:,1), interp_points_RC(:,2), interp_points_RC(:,3));
scatter3(interp_points_RC(:,1), interp_points_RC(:,2), interp_points_RC(:,3));

%% Peripheral processes

% Calculate interpolated points
px = PP_points(:,1);
py = PP_points(:,2);
pz = PP_points(:,3);
interp_points_PP = interparc(fibre_count, px, py, pz);

% Plot PP
plot3(interp_points_PP(:,1), interp_points_PP(:,2), interp_points_PP(:,3));
scatter3(interp_points_PP(:,1), interp_points_PP(:,2), interp_points_PP(:,3));

%% Axon start

% Calculate interpolated points
px = A1_points(:,1);
py = A1_points(:,2);
pz = A1_points(:,3);
scatter3(A1_points(:,1), A1_points(:,2), A1_points(:,3));
interp_points_A1 = interparc(fibre_count, px, py, pz);

% Plot A1
plot3(interp_points_A1(:,1), interp_points_A1(:,2), interp_points_A1(:,3));
% scatter3(interp_points_A1(:,1), interp_points_A1(:,2), interp_points_A1(:,3));

%% Axon, near bend

% Calculate interpolated points
px = A2_points(:,1);
py = A2_points(:,2);
pz = A2_points(:,3);
scatter3(A2_points(:,1), A2_points(:,2), A2_points(:,3));
interp_points_A2 = interparc(fibre_count, px, py, pz);

% Plot A2
plot3(interp_points_A2(:,1), interp_points_A2(:,2), interp_points_A2(:,3));
% scatter3(interp_points_A2(:,1), interp_points_A2(:,2), interp_points_A2(:,3));

%% Axon end offset

% Calculate interpolated points
px = A3_points(:,1);
py = A3_points(:,2);
pz = A3_points(:,3);
interp_points_A4 = interparc(fibre_count, px, py, pz);

% Plot A3
plot3(interp_points_A4(:,1), interp_points_A4(:,2), interp_points_A4(:,3));
scatter3(interp_points_A4(:,1), interp_points_A4(:,2), interp_points_A4(:,3));

%% Axon end

% Calculate interpolated points
px = A4_points(:,1);
py = A4_points(:,2);
pz = A4_points(:,3);
interp_points_A3 = interparc(fibre_count, px, py, pz);

% Plot A4
plot3(interp_points_A3(:,1), interp_points_A3(:,2), interp_points_A3(:,3));
scatter3(interp_points_A3(:,1), interp_points_A3(:,2), interp_points_A3(:,3));

%% MAKE NERVE FIBRES

% Initialise array for trim_after points
trim_after = zeros(fibre_count, 1);

% Initialise arrays for export node count
nodes_per_fibre_PP_RC = zeros(fibre_count, 1);
nodes_per_fibre_RC_AN = zeros(fibre_count, 1);

% For each fibre
for i = 1:fibre_count
    
    % TODO: Add logic for more control points for upper level fibres
    % Get key points
    fibre_key_points_spiral = vertcat(interp_points_PP(i,:), interp_points_RC(i,:), interp_points_A1(i,:), interp_points_A2(i,:), interp_points_A4(i,:), interp_points_A3(i,:));
    
    % Interpolate additional nodes for rough fibre (overall path)
    fibre_approx = interparc(nodes_per_fibre_1, fibre_key_points_spiral(:,1), fibre_key_points_spiral(:,2), fibre_key_points_spiral(:,3));
    
    % Find "trim point" at RC
    for j = 1:size(fibre_approx,1)-1    % Where 1 is the peripheral end
%         scatter3(periph_fibre(j,1), periph_fibre(j,2), periph_fibre(j,3));
        
        dist_1 = sqrt((fibre_approx(j,1)-interp_points_RC(i,1))^2 + (fibre_approx(j,2)-interp_points_RC(i,2))^2 + (fibre_approx(j,3)-interp_points_RC(i,3))^2);
        dist_2 = sqrt((fibre_approx(j+1,1)-interp_points_RC(i,1))^2 + (fibre_approx(j+1,2)-interp_points_RC(i,2))^2 + (fibre_approx(j+1,3)-interp_points_RC(i,3))^2);
        dist_R = sqrt((fibre_approx(j+1,1)-fibre_approx(j,1))^2 + (fibre_approx(j+1,2)-fibre_approx(j,2))^2 + (fibre_approx(j+1,3)-fibre_approx(j,3))^2);

        if and(dist_1 < dist_R, dist_2 < dist_R)
%             fprintf(['Split fibre ' num2str(i) ' after node ' num2str(j)]);
%             fprintf('\n');
            trim_after(i) = j;
        end
    end
            
    % Split into two splines (PP-RC, RC-AN) at Rosenthal's canal
    fibre_PP_RC_points = zeros(trim_after(i)+1, 3);
    fibre_PP_RC_points(1,:) = interp_points_RC(i,:); % First point is RC
    
    for k = 2:size(fibre_PP_RC_points,1);
        fibre_PP_RC_points(k,:) = fibre_approx(trim_after(i)+2-k,:);
    end
    
    fibre_PP_RC = interparc(nodes_per_fibre_1, fibre_PP_RC_points(:,1), fibre_PP_RC_points(:,2), fibre_PP_RC_points(:,3));
%     plot3(fibre_PP_RC(:,1), fibre_PP_RC(:,2), fibre_PP_RC(:,3));
%     scatter3(fibre_PP_RC(:,1), fibre_PP_RC(:,2), fibre_PP_RC(:,3));
    
    fibre_RC_AN_points = zeros(nodes_per_fibre_1 - trim_after(i) + 1, 3);
    fibre_RC_AN_points(1,:) = interp_points_RC(i,:); % First point is RC
    
    for k = 2:size(fibre_RC_AN_points,1);
        fibre_RC_AN_points(k,:) = fibre_approx(trim_after(i)-1+k,:);
    end
    
    fibre_RC_AN = interparc(nodes_per_fibre_1, fibre_RC_AN_points(:,1), fibre_RC_AN_points(:,2), fibre_RC_AN_points(:,3));
%     plot3(fibre_RC_AN(:,1), fibre_RC_AN(:,2), fibre_RC_AN(:,3));
%     scatter3(fibre_RC_AN(:,1), fibre_RC_AN(:,2), fibre_RC_AN(:,3));
    
%     if i==4 % Temp, for testing
        % Adjust length to closest multiple of Ranvier spacing
        % Get length of existing arc
        fibre_length_orig = arclength(fibre_PP_RC(:,1), fibre_PP_RC(:,2), fibre_PP_RC(:,3));
        fprintf(['Arc length at fibre ' num2str(i) ': %0.3f mm '], fibre_length_orig);

        whole_divs = floor(fibre_length_orig / internode_ranvier_spacing);
        part_divs = mod(fibre_length_orig, internode_ranvier_spacing) / internode_ranvier_spacing;

        % Make a temporary high resolution fibre
        % XXX: Using 200 times the number of original nodes
        fibre_temp = interparc(accuracy * nodes_per_fibre_1, fibre_PP_RC_points(:,1), fibre_PP_RC_points(:,2), fibre_PP_RC_points(:,3));
%         scatter3(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));

        fibre_length = arclength(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
        target_fibre_length = internode_ranvier_spacing * whole_divs;

        % Trim by progressively removing data points
        while (fibre_length > target_fibre_length)
            fibre_temp = fibre_temp(1:(size(fibre_temp,1)-1),:);
            fibre_length = arclength(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
        end
        
        shorter_by = fibre_length_orig - fibre_length;
        fprintf('(shortened by: %0.3f mm)\n', shorter_by);
%         scatter3(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));

        % Interpolate new spline with spacing based on nodes of Ranvier separation
        fibre_PP_RC = interparc(whole_divs+1, fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
        scatter3(fibre_PP_RC(:,1), fibre_PP_RC(:,2), fibre_PP_RC(:,3));
        plot3(fibre_PP_RC(:,1), fibre_PP_RC(:,2), fibre_PP_RC(:,3));

%     end
    
%     if i==5 % Temp, for testing
        % Adjust length to closest multiple of Ranvier spacing
        % Get length of existing arc
        fibre_length_orig = arclength(fibre_RC_AN(:,1), fibre_RC_AN(:,2), fibre_RC_AN(:,3));
        fprintf(['Arc length at fibre ' num2str(i) ': %0.3f mm '], fibre_length_orig);

        whole_divs = floor(fibre_length_orig / internode_ranvier_spacing);
        part_divs = mod(fibre_length_orig, internode_ranvier_spacing) / internode_ranvier_spacing;

        % Make a temporary high resolution fibre
        % XXX: Using 200 times the number of original nodes
        fibre_temp = interparc(accuracy * nodes_per_fibre_1, fibre_RC_AN_points(:,1), fibre_RC_AN_points(:,2), fibre_RC_AN_points(:,3));
%         scatter3(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));

        fibre_length = arclength(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
        target_fibre_length = internode_ranvier_spacing * whole_divs;

        % Trim by progressively removing data points
        while (fibre_length > target_fibre_length)
            fibre_temp = fibre_temp(1:(size(fibre_temp,1)-1),:);
            fibre_length = arclength(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
        end
        
        shorter_by = fibre_length_orig - fibre_length;
        fprintf('(shortened by: %0.3f mm)\n', shorter_by);
%         fprintf('Elapsed time: %d minutes and %0.1f seconds.\n\n', mins, secs);

%         scatter3(fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));

        % Interpolate new spline with spacing based on nodes of Ranvier separation
        fibre_RC_AN = interparc(whole_divs+1, fibre_temp(:,1), fibre_temp(:,2), fibre_temp(:,3));
%         scatter3(fibre_RC_AN(:,1), fibre_RC_AN(:,2), fibre_RC_AN(:,3));
        plot3(fibre_RC_AN(:,1), fibre_RC_AN(:,2), fibre_RC_AN(:,3));

%     end
    
    % Add node coordinates to CSV matrix
    
end

% Export CSV

%% End timer
timer_end = toc(timer_start);

hours = floor(timer_end/3600);
mins = floor((timer_end-hours*3600)/60);
secs = rem(timer_end,60);

%% Mark end of script
fprintf('\n');
fprintf(['SCRIPT COMPLETED']);
fprintf('\n');
if (hours == 0)
%     fprintf(['Time taken: ' num2str(mins) ' mins ' num2str(secs) ' secs ']);
	fprintf('Time taken: %d minutes and %0.2f seconds.\n\n', mins, secs);
else
    fprintf('Time taken: %d hours, %d minutes and %0.2f seconds.\n\n', hours, mins, secs);
end