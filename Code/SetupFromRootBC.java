...
public class SetupFromRootBC {
	...
	public static Model run() {
		...
		// XXX: Manual inputs
			...
			final String[] terminalSelects = {"sel1", "sel2", "sel3", "sel4", "sel5", "sel6", "sel7", "sel8"};
			final String[] boundaryTags = {"hemi_gnd", "hemi_off", "inf_gnd", "sph_gnd", "nrvD_gnd", "caud_gnd"};
//			final String[] boundaryTags = {"cube_r5", "b_box"};
//			final String[] boundaryTags = {"sph_r20", "sph_r10", "sph_r5", "nrv_gnd"};
			
			// Only list entities of interest
			// (These are the tags from COMSOL)
			String[] terminalTags = {"term1", "term2", "term3", "term4", "term5", "term6", "term7", "term8"};  
			String[] boundaries = {"gnd1"}; // , "term9", "gnd5", "gnd2", "gnd3", "gnd4"}; 
//			String[] boundaries = {"gnd1"}; // , "gnd2", "gnd3", "gnd4"};
...
//		model.variable("var5").set("vessels", "allBV");
		...
		for (int j = 0; j < terminalTags.length; j++) {
			...
			for (int i = 0; i < boundaries.length; i++) {
			    ...
			    model.physics("ec").feature(activeBC).active(true);
				    ...
				    if (activeBC.equalsIgnoreCase("gnd5")) {
				    	// Disable insulation
				        model.physics("ec").feature("ein3").active(false);
				    }
			    ...
			    model.physics("ec").feature(activeBC).active(false); // Deactivate ground
			    	...
			    	if (activeBC.equalsIgnoreCase("gnd5")) {
				    	// Re-enable insulation
				        model.physics("ec").feature("ein3").active(true);
				    }
			}
		}
